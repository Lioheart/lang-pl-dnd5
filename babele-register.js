...
/**
 * Zabezpieczenie dla advancement D&D 5e.
 *
 * Niektóre okna Advancement pobierają tablicę system.advancement już po
 * przetworzeniu dokumentu kompendium. Hook jawnie nanosi tłumaczenia title
 * i hint na wpisy rozpoznawane po _id, oryginalnym title albo indeksie.
 */
Hooks.on("babele.translateDocumentData", (context) => {
    if (context.metadata?.type !== "Item") return;

    const sourceAdvancement = foundry.utils.getProperty(
        context.source,
        "system.advancement"
    );

    const advancementTranslations = context.translation?.advancement;

    if (!Array.isArray(sourceAdvancement)) return;

    if (
        !advancementTranslations
        || typeof advancementTranslations !== "object"
    ) {
        return;
    }

    const currentlyTranslated = foundry.utils.getProperty(
        context.translated,
        "system.advancement"
    );

    /*
     * Zachowujemy wynik wbudowanego konwertera structured, jeżeli już
     * utworzył tablicę. Dzięki temu nie tracimy innych zmian, np. jednostek.
     */
    const baseAdvancement = Array.isArray(currentlyTranslated)
        ? currentlyTranslated
        : sourceAdvancement;
    ...
    const findTranslation = (sourceEntry, index) => {
        if (Array.isArray(advancementTranslations)) {
            return advancementTranslations[index];
        }
        ...
        for (const key of candidates) {
            if (key && hasOwn(advancementTranslations, key)) {
                return advancementTranslations[key];
            }
        }
    ...
    const translatedAdvancement = sourceAdvancement.map(
        (sourceEntry, index) => {
            const currentEntry = baseAdvancement[index] ?? sourceEntry;
            const translation = findTranslation(sourceEntry, index);

            if (!translation || typeof translation !== "object") {
                return currentEntry;
            }

            const updated = foundry.utils.deepClone(currentEntry);
            ...
            if (
                Array.isArray(updated.configuration?.choices)
                && choiceTranslations
            ) {
                updated.configuration.choices =
                    updated.configuration.choices.map(
                        (choice, choiceIndex) => {
                            ...
                            if (typeof choiceTranslation === "string") {
                                if ("label" in translatedChoice) {
                                    translatedChoice.label =
                                        choiceTranslation;
                                }

                                if ("title" in translatedChoice) {
                            ...
                            for (
                                ...
                                if (
                                    typeof choiceTranslation[key]
                                    === "string"
                                ) {
                                    translatedChoice[key] =
                                        choiceTranslation[key];
                                }
                            }
